...
- **A GitHub Actions run has not actually executed yet** — this repository isn't pushed to GitHub yet (Task 4.1). Once pushed with `DOCKERHUB_USERNAME`/`DOCKERHUB_TOKEN` secrets configured, `push origin main` will trigger the pipeline. *Screenshot of the successful run to be added here after that push, per the submission checklist.*
